...
def write_backend_override(
    workspace_dir, provisioned_product_descriptor, state_bucket, state_region
):
    backend_override = {
        "terraform": {
            "backend": {
...
def write_variable_override(workspace_dir, variables):
    ...
    for variable in variables:
        try:
            ...
            variable_override["variable"][variable["key"]] = {
                "default": variable_value_json
            }
...
def write_provider_override(
    workspace_dir, provisioned_product_descriptor, launch_role_arn, region, tags
):
    provider_override = {
        "provider": {
            "aws": {
                ...
                "assume_role": {
                    ...
                    "session_name": __format_session_name(
                        provisioned_product_descriptor
                    ),
                },
                "default_tags": {"tags": {}},
            }
    ...
    if tags != None:
        for tag in tags:
            key = tag["key"]
            provider_override["provider"]["aws"]["default_tags"]["tags"][f"{key}"] = (
                tag["value"]
            )
