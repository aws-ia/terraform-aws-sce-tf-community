...
def __start_state_machine(record: dict, state_machine_arn: str):
    ...
    log.info(
        f"Starting state machine {state_machine_arn} with token {token} & name: {execution_name} & payload: {state_machine_payload}"
    )
    ...
    start_execution_request_id: str = start_execution_response[RESPONSE_METADATA_KEY][
        REQUEST_ID_KEY
    ]

    log.info(
        f"Started state machine execution with arn: {execution_arn} for request Id: {start_execution_request_id}"
    )
...
def handle_sqs_records(event, context):
    ...
    for record in records:
        ...
        try:
            ...
            failing_request_id: str = clientError.response[RESPONSE_METADATA_KEY][
                REQUEST_ID_KEY
            ]

            if error_code == EXECUTION_ALREADY_EXISTS:
                log.warning(
                    f"A state machine execution with the same execution ARN "
                    f"already exists for requestId: {failing_request_id} & record {record}"
                )
            else:
                log.error(
                    f"Processing for record: {record} failed with error: {clientError} "
                    f"and requestId: {failing_request_id}"
                )
                batchItemFailures[BATCH_ITEM_FAILURES_KEY].append(
                    {ITEM_IDENTIFIER_KEY: record[MESSAGE_ID_KEY]}
                )

        except Exception as exception:
            log.error(
                f"Processing for {record} failed with error: {exception} & stack trace: {traceback.format_exc()}"
            )
            batchItemFailures[BATCH_ITEM_FAILURES_KEY].append(
                {ITEM_IDENTIFIER_KEY: record[MESSAGE_ID_KEY]}
            )
