...
if LAMBDA_TASK_ROOT in os.environ:
    ...
    log.info(
        f"{LAMBDA_TASK_ROOT} was found in os.environ: {lambda_task_root}. Now modifying sys.path to use the local versions of boto3/botocore instead of the versions provided by the Lambda runtime environment."
    )
    ...
    log.info(
        f"{LAMBDA_TASK_ROOT} was not found in os.environ. No modifications were made to sys.path to load the local versions of boto3/botocore. this indicates that this code is not running in a Lambda environment."
    )
...
class ServiceCatalogFacade:
    ...
    def __get_service_catalog_client(self, app_config: Configuration):
        verify = (
            SERVICE_CATALOG_VERIFY_SSL not in os.environ
            or os.environ[SERVICE_CATALOG_VERIFY_SSL] == TRUE
        )
        log.info(f"Constructing Service Catalog client: verify={verify}")

        if (
            SERVICE_CATALOG_ENDPOINT in os.environ
            and os.environ[SERVICE_CATALOG_ENDPOINT]
        ):
            endpoint_url = os.environ[SERVICE_CATALOG_ENDPOINT]
            log.info(
                f"Constructing Service Catalog client with overridden endpoint={endpoint_url}"
            )
            return boto3.client(
                "servicecatalog",
                verify=verify,
            ...
            return boto3.client(
                "servicecatalog", verify=verify, config=app_config.get_boto_config()
            )

    def __log_response(self, response):
        log.info(
            f"Notified service catalog of workflow results via request Id: "
            f'{response["ResponseMetadata"]["RequestId"]}'
        )

    def notify_provision_succeeded(
        self,
        *,
        workflow_token: str,
        record_id: str,
        ...
        response = self.__service_catalog_client.notify_provision_product_engine_workflow_result(
            ...
            ResourceIdentifier={
                "UniqueTag": {"Key": tracer_tag_key, "Value": tracer_tag_value}
            },
            Outputs=outputs,
    ...
    def notify_update_succeeded(
        self, *, workflow_token: str, record_id: str, outputs: list
    ):
    ...
    def notify_update_failed(
        self, *, workflow_token: str, record_id: str, failure_reason: str, outputs: str
    ):
    ...
    def notify_terminate_succeeded(
        self,
        *,
        workflow_token: str,
        record_id: str,
    ...
    def notify_terminate_failed(
        self, *, workflow_token: str, record_id: str, failure_reason: str
    ):
