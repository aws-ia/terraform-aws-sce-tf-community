...
from terraform_runner.override_manager import (
    write_backend_override,
    write_provider_override,
    write_variable_override,
)
...
def __parse_arguments():
    ...
    parser.add_argument(
        "--action", help="The action to perform", choices=[APPLY_ACTION, DESTROY_ACTION]
    )
    parser.add_argument(
        "--provisioned-product-descriptor",
...
def __write_common_overrides(workspace_dir, args):
    write_backend_override(
        workspace_dir,
        args.provisioned_product_descriptor,
        args.terraform_state_bucket,
        args.region,
    )
    write_provider_override(
        workspace_dir,
